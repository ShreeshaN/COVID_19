...
from covid_19.datagen.vggish import vggish_params as params
...
def define_vggish_slim(training=False):
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                        ...
                        biases_initializer=tf.compat.v1.zeros_initializer(), activation_fn=tf.compat.v1.nn.relu,
                        trainable=training), slim.arg_scope([slim.conv2d], kernel_size=[3, 3], stride=1,
                                                            padding='SAME'), slim.arg_scope([slim.max_pool2d],
                                                                                            kernel_size=[2, 2],
                                                                                            stride=2,
                                                                                            padding='SAME'), tf.compat.v1.variable_scope(
            'vggish'):
        # Input: a batch of 2-D log-mel-spectrogram patches.

        features = tf.compat.v1.placeholder(

                tf.compat.v1.float32, shape=(None, params.NUM_FRAMES, params.NUM_BANDS),

                name='input_features')

        # Reshape to 4-D so that we can convolve a batch with conv2d().

        net = tf.compat.v1.reshape(features, [-1, params.NUM_FRAMES, params.NUM_BANDS, 1])
        ...
        return tf.compat.v1.identity(net, name='embedding')
